...
const packageName = 'react-native-event';
const source = readFileSync(join(repository, 'test/types/src/index.ts'), 'utf8');
...
for (const profile of generated ? ['current'] : ['minimum', 'current']) {
  ...
  try {
    ...
    const regression = source.replace("const invalidProps: EventProviderProps = { events: ['press'] };", 'const invalidProps: EventProviderProps = { children: null };');
    if (regression === source) throw new Error('Negative consumer assertion was not found.');
    writeFileSync(join(fixture, 'src/index.ts'), regression);
